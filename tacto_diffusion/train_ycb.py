...
from data.dataio_ycb import Diffusion_Dataset
...
def load_data(image_size: int, remove_bg: bool, batch_sz: int):
    ...
    dataset_names_train = sorted(glob.glob("{0}/*".format(DIRS["data"])))
    for dataset_name in dataset_names_train:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--config",
        type=str,
        default="config/config_ycb_slide.json",
    ...
    DIRS = DIRS_YCB
    split_ycb_dataset(test_ratio=conf_params["dataset"]["test_ratio"])
