...
def extract_poses_real(
    ...
    _, idx_removed = clean_up_optitrack(gt_p_cam)  # clean up jumps
    ...
    return gt_p_cam, gt_p, idx_removed
...
def clean_up_optitrack(poses):
    ...
    return filtered_poses, idx_removed
...
def digit_poses_tf(
    ...
    gt_p_cam, gt_p, meas_p = (
        torch.tensor(camposes),
        torch.tensor(gelposes),
        torch.tensor(gelposes_meas),
    )
    gt_p_cam, gt_p, meas_p = (
        xyzw_to_wxyz(gt_p_cam),
        xyzw_to_wxyz(gt_p),
